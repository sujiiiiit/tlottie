...
export class LottieLoader {
  private loadPromise: Promise<void> = !IS_WEB_ASSEMBLY_SUPPORTED
    ? Promise.reject(makeError("NO_WASM"))
    : Promise.resolve();
  ...
  public loadLottieWorkers() {
    ...
    return (this.loadPromise = new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < this.workersLimit; ++i) {
        ...
        queryableWorker.addEventListener(
          "ready",
          () => {
  ...
  public loadAnimationAsAsset(
    params: Omit<RLottieOptions, "animationData" | "name">,
  ...
  public loadAnimationDataFromURL(
    url: string,
    method: "json" | "blob" = "blob"
  ): Promise<Blob | any> {
  ...
  public loadAnimationFromURLManually(name: LottieAssetName) {
    ...
    return this.loadAnimationDataFromURL(url).then((blob) => {
      return (params: Omit<RLottieOptions, "animationData">) =>
        this.loadAnimationFromURLNext(blob, params, url);
    });
  }

  public loadAnimationFromURL(
    params: Omit<RLottieOptions, "animationData">,
    url: string
  ) {
    return this.loadAnimationDataFromURL(url).then((blob) => {
  ...
  public loadAnimationFromURLNext(
    blob: Blob,
    params: Omit<RLottieOptions, "animationData">,
    url: string
  ) {
  ...
  public async loadAnimationWorker(
    ...
    if (params.sync) {
      ...
      if (players?.size) {
        const player = players?.entries().next().value?.[0] ?? null;
        if (player === null) {
  ...
  private onPlayerLoaded = (reqId: number, frameCount: number, fps: number) => {
    ...
    if (!player) {
      console.warn("onPlayerLoaded on destroyed player:", reqId, frameCount);
      return;
    }

    console.debug("onPlayerLoaded");
  ...
  private onFrame = (
    reqId: number,
    frameNo: number,
    frame: Uint8ClampedArray | ImageBitmap
  ) => {
    const player = this.players[reqId];
    if (!player) {
      console.warn("onFrame on destroyed player:", reqId, frameNo);
      return;
    }
  ...
  private initPlayer(el: RLottiePlayer["el"], options: RLottieOptions) {
    ...
    const playersByCacheName = cacheName
      ? (this.playersByCacheName[cacheName] ??= new Set())
      : undefined;
    if (playersByCacheName) {
    ...
    player.addEventListener("destroy", () => {
      ...
      if (playersByCacheName && playersByCacheName.delete(player) && !playersByCacheName.size) {
