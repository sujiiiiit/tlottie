...
export class RLottieIconItem implements RLottieIconItemOptions {
  public name: LottieAssetName = '' as LottieAssetName;
  public parts: RLottieIconItemPart[] = [];
  public player!: RLottiePlayer;
  public initFrame: number = 0;
  public autoplay: boolean = false;
  public color!: RLottieColor;
  public loadPromise: Promise<void> = Promise.resolve();
...
export default class RLottieIcon {
  ...
  protected skipAnimation: boolean = false;
  ...
  public getItem(name?: LottieAssetName): RLottieIconItem {
    const item = !name && this.items.size === 1 ? this.items.values().next().value : (name ? this.items.get(name) : undefined);
    if (!item) {
