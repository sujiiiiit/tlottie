...
export default class RLottiePlayer extends EventListenerBase<{
  ...
  private frInterval: number = 0;
  private frThen: number = 0;
  private rafId: number | undefined = 0;
  ...
  public clamped: Uint8ClampedArray = new Uint8ClampedArray();
  ...
  public minFrame: number = 0;
  public maxFrame: number = 0;

  private playedTimes = 0;

  private currentMethod: RLottiePlayer['mainLoopForwards'] | RLottiePlayer['mainLoopBackwards'] = this.mainLoopForwards;
  private frameListener: (currentFrame: number) => void = () => {};
  private skipFirstFrameRendering: boolean = false;
  ...
  constructor({el, worker, options}: {
    ...
    // ! :(
    this.initFrame = options.initFrame ?? 0;
    this.color = options.color;
    this.textColor = options.textColor;
    this.name = options.name ?? '';
    this.skipFirstFrameRendering = options.skipFirstFrameRendering ?? false;
    this.toneIndex = options.toneIndex ?? 0;
    this.raw = false;
    this.liteModeKey = options.liteModeKey ?? 'defaultLiteModeKey' as LiteModeKey;
    ...
    this.contexts = this.canvas.map((canvas) => canvas.getContext('2d')).filter((context): context is CanvasRenderingContext2D => context !== null);
  ...
  public applyColor(context: CanvasRenderingContext2D) {
    applyColorOnContext(
      ...
      this.color || (this.textColor ? customProperties.getPropertyAsColor(this.textColor) : '#000000'),
      0,
      0,
      this.width,
      this.height
  ...
  public renderFrame2(frame: Uint8ClampedArray | HTMLCanvasElement | ImageBitmap, frameNo: number) {
    ...
    try {
      ...
      this.contexts.forEach((context, idx) => {
        let cachedSource: HTMLCanvasElement | ImageBitmap | undefined = this.cache.framesNew.get(frameNo);
        if(!(frame instanceof Uint8ClampedArray)) {
          cachedSource = frame;
        } else if(idx > 0) {
