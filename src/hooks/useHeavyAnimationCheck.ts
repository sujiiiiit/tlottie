...
if (heavyAnimationPromise && heavyAnimationPromise.resolve) {
  heavyAnimationPromise.resolve();
}

export function dispatchHeavyAnimationEvent(
  promise: Promise<any>,
  timeout?: number
) {
  if (!isAnimating) {
  ...
  ++promisesInQueue;
  console.log("attach promise, length:", promisesInQueue, timeout);
  ...
  Promise.race(promises).then(() => {
    if (
      heavyAnimationPromise !== _heavyAnimationPromise ||
      heavyAnimationPromise.isFulfilled
    ) {
      // interrupted
      return;
    }
...
export default function (
  ...
  const add = listenerSetter
    ? listenerSetter.add(eventListener)
    : eventListener.addEventListener.bind(eventListener);
  const remove = listenerSetter
    ? listenerSetter.removeManual.bind(listenerSetter, eventListener)
    : eventListener.removeEventListener.bind(eventListener);
